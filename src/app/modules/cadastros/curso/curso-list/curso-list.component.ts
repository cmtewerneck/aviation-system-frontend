...
export class CursoListComponent implements OnInit, OnDestroy {
    ...
    load() {
        this.isLoading = true;
        this._cursoService.list(this.query).subscribe(result => {
    ...
    delete(id: string) {
        this.isLoading = true;
        this._cursoService.delete(id).subscribe(_ => {
    ...
    exportToExcel() {
        this._cursoService.exportToExcel().subscribe(res => {
            console.log(res);
        }, error => {
            console.log(error);
        });
    ...
    exportToPdf() {
        this._cursoService.exportToPdf().subscribe(res => {
            console.log(res);
        }, error => {
            console.log(error);
        });
    ...
    openDialogDelete(id): void {
        const dialogRef = this.dialog.open(CursoModalExcluirComponent, {
          width: '500px',
          data: { id: id }
        });
