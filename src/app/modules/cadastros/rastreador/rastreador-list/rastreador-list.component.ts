...
@Component({
    ...
    templateUrl: './rastreador-list.component.html',
    styleUrls: ['./rastreador-list.component.scss'],
    encapsulation: ViewEncapsulation.None,
})

export class RastreadorListComponent implements OnInit, OnDestroy {
    ...
    productsTableColumns: string[] = ['codigo', 'modelo', 'matricula', 'actions'];
    ...
    load() {
        this.isLoading = true;
        this._rastreadorService.list(this.query).subscribe(result => {
            this.rastreadores = result.data;
    ...
    delete(id: string) {
        this.isLoading = true;
        this._rastreadorService.delete(id).subscribe(_ => {
    ...
    filter() {
        ...
        if (filter.modelo?.trim()) {
            this.query.filters.push(new FilterInfo('modelo', FieldTypeEnum.String, FilterOperatorEnum.Contains, filter.modelo.trim()));
        }
    ...
    openDialogExcel(): void {
        const dialogRef = this.dialog.open(RastreadorModalExcelComponent, {
          width: '500px',
        });
    ...
    openDialogPdf(): void {
        const dialogRef = this.dialog.open(RastreadorModalPdfComponent, {
          width: '500px',
        });
