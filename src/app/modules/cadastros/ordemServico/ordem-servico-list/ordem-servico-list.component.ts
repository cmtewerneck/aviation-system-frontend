...
@Component({
    ...
    styleUrls: ['./ordem-servico-list.component.scss'],
    encapsulation: ViewEncapsulation.None,
})

export class OrdemServicoListComponent implements OnInit, OnDestroy {
    ...
    productsTableColumns: string[] = ['numeroOrdem', 'tipo', 'dataAbertura', 'dataFechamento', 'matricula', 'actions'];
    ...
    load() {
        this.isLoading = true;
        this._ordemServicoService.list(this.query).subscribe(result => {
    ...
    delete(id: string) {
        this.isLoading = true;
        this._ordemServicoService.delete(id).subscribe(_ => {
    ...
    filter() {
        ...
        if (filter.numeroOrdem?.trim()) {
            this.query.filters.push(new FilterInfo('numeroOrdem', FieldTypeEnum.String, FilterOperatorEnum.Contains, filter.numeroOrdem.trim()));
        }
        ...
        if (filter.tipo?.trim()) {
            this.query.filters.push(new FilterInfo('tipo', FieldTypeEnum.String, FilterOperatorEnum.Contains, filter.tipo.trim()));
        }
