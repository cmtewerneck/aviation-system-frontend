...
@Component({
    selector: 'app-nec-list',
    templateUrl: './nec-list.component.html',
    styleUrls: ['./nec-list.component.scss'],
    encapsulation: ViewEncapsulation.None,
})

export class NecListComponent implements OnInit, OnDestroy {
    ...
    productsTableColumns: string[] = ['numero', 'equipamento', 'dataExame', 'tipoTreinamento', 'situacaoTreinamento', 'numeroSae', 'actions'];
    ...
    load() {
        this.isLoading = true;
        this._necService.list(this.query).subscribe(result => {
            this.necs = result.data;
    ...
    delete(id: string) {
        this.isLoading = true;
        this._necService.delete(id).subscribe(_ => {
    ...
    filter() {
        ...
        if (filter.equipamento?.trim()) {
            this.query.filters.push(new FilterInfo('equipamento', FieldTypeEnum.String, FilterOperatorEnum.Contains, filter.equipamento.trim()));
        }

        if (filter.numeroSae?.trim()) {
            this.query.filters.push(new FilterInfo('numeroSae', FieldTypeEnum.String, FilterOperatorEnum.Contains, filter.numeroSae.trim()));
        }
    ...
    openDialogExcel(): void {
        const dialogRef = this.dialog.open(NecModalExcelComponent, {
          width: '500px',
        });
    ...
    openDialogPdf(): void {
        const dialogRef = this.dialog.open(NecModalPdfComponent, {
          width: '500px',
        });
